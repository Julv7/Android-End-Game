...
public class MonthView extends Fragment {
    ...
    private void update(int year, int month) {
        new Thread(() -> {
            ...
            for (int i = 0; i < 31; i++) {
                long yit = DatabaseAction.getInstance(getContext()).getAllIncomesDao().getDayI(account,year, month, i + 1);
                long yot = DatabaseAction.getInstance(getContext()).getAllIncomesDao().getDayO(account,year, month, i + 1);
                mi += yit;
                mo += yot;
