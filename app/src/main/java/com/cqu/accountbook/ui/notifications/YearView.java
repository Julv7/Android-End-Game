...
public class YearView extends Fragment {
    ...
    private void update(int year) {
        new Thread(() -> {
            ...
            for (int i = 0; i < 12; i++) {
                long yit = DatabaseAction.getInstance(getContext()).getAllIncomesDao().getMonthI(account,year, i + 1);
                long yot = DatabaseAction.getInstance(getContext()).getAllIncomesDao().getMonthO(account,year, i + 1);
                yi += yit;
                yo += yot;
